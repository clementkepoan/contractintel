...
def build_answer_instructions(intents: set[str], output_language: str) -> str:
    lines = [
        ...
        "如果不同條款提供不同權利、救濟或後果，必須全部列出，不可只挑最高分條款。",
        "每一個結論後面都要標明對應條款。",
    ...
    if "action" in intents or "progress_delay" in intents:
        lines.extend(
            [
                "這是一個『可採取哪些行動／救濟』問題。",
                "必須逐項列出所有可主張的行動，例如：暫停付款、扣罰違約金、終止或解除契約、另覓廠商、扣抵款項、請求損害賠償。",
                "如果多個條款都成立，必須合併整理成完整清單，不可只回答其中一條。",
                "若某行動有門檻條件，請明確寫出門檻與本題事實是否符合。",
                "回答格式優先使用：『可採取的行動』清單，接著補充『條款依據』與『限制／條件』。",
            ]
        )
